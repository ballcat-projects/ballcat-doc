...
- 🌟 `PageParam` 分页上限校验改为动态配置，默认 100,  通过调整 `ballcat.web.page-size-limit` 配置可以修改

- 🌟 所有分页接口，添加 PageParam 的入参校验

- 🌟 支持带表别名的排序列

- 🌟 `PageParamRequest` 使用 sort 接收多列排序属性（使用 spring Pageable 的默认规则），前端传参更直观方便
...
- 🌟 新增组织机构列表查询
- 🌟 菜单和组织机构的筛选都移交到前端处理了
- :art: 解耦 system 和 websocket 模块（notify 解耦未完成，后续重构通知时处理）
...
- 🌟 websocket 的 session 存储角色，从 `WebSocketSessionHolder` 修改为 `WebSocketSessionStore`, 方便用户替换
- 🌟 现在不再默认覆盖同一 sessionKey 的 wsSession 了，方便用户进行全客户端推送
- 🌟 新增 `AbstractMessageDistributor`， 下沉 `MessageDistributor` 接口中的 default 方法，方便用户使用继承的方式替换
- 🌟 跨域配置添加 `allowedOriginPatterns` 属性
- 🌟 添加 sockjs 的支持
- 🎨 优化下 `RedisMessageListener` 的注册方式，防止误扫描导致的 bean 注册异常
- 🎨 优化 starter 的配置类结构
- :art: 精简代码层级，将接口 `JsonWebSocketMessage` 删除，原抽象类 `AbstractJsonWebSocketMessage` 改名为 `JsonWebSocketMessage`
...
- :white_check_mark: 添加幂等组件的一些测试用例



#### 接口文档 ballcat-extend-openapi

- :sparkles: 添加部分 OpenAPI 配置
- :sparkles: 文档基础信息的配置添加一个 info 层级，以便将 info 的 extensions 和 OpenAPI 的 extensions 区分开
